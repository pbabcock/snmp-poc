...
class Mib(object):
    ...
    def getSmokeDescription(self):
        return "Smoke Test Status Mib"

    def getSmokeStatus(self):
        with self._lock:
            return self._test_count

    def setSmokeStatus(self, value):
        with self._lock:
...
class SNMPAgent(object):
    ...
    def __init__(self, mibObjects):
        ...
        config.addV1System(self._snmpEngine, "agent", "$mtSNMPr%")
    ...
    def setTrapReceiver(self, host, community):
        ...
        config.addNotificationTarget(
            self._snmpEngine, 'smoke-test-failure-notification', 'my-filter',
...
class Worker(threading.Thread):
    ...
    def run(self):
        while True:
            time.sleep(30)
            mib.setSmokeStatus(1000)
            agent.sendTrap()

if __name__ == '__main__':
    mib = Mib()
    objects = [MibObject('SMOKE-MIB', 'smokeDescription', mib.getSmokeDescription),
               MibObject('SMOKE-MIB', 'smokeStatus', mib.getSmokeStatus)]
    agent = SNMPAgent(objects)
    agent.setTrapReceiver('10.200.23.6 ', 'traps')
